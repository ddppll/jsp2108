...
<%
	String mid = session.getAttribute("sAdmin") == null ? "" : (String) session.getAttribute("sAdmin");//sAdmin은 객체라 문자열로 바꿔줌
	
	GuestDAO dao = new GuestDAO();
	GuestVO vo = new GuestVO();

	/* 페이징 처리
	   1. 한 페이지 분량을 정한다. : pageSize = 10건(관리자가 정한다)
	   2. 총 레코드 건수를 구한다. : totRecCnt => SQL 함수 중 count(*) 함수 사용
	   3. 총 페이지 수를 구한다. : totPage => 총 레코드수/페이지분량으로 하되, 나머지각 0이면 몫을, 0이 아니면 몫+1로 페이지수를 설정
	   4. 현재 페이지의 시작 index 번호를 구한다. : startIndexNo =>  (현재페이지-1) * 페이지분량
	   5. 현재 화면에 보이는 방명록 시작 번호 : curScrStarNo = 총 레코드수 - 현재 페이지의 시작 indx 번호

...
-- irum필드를 name필드로 수정하시오? 길이는 가변문자형(20), null 값 불허
...
select name as '성명', gender as '성별', jikwi as '직위', address as '주소' from insarok;
...
-- 서울에 사는 여자직원을 보여주시오?
select * from insarok where gender='여자' and address='서울';
...
-- '영업과', '남자' 직원 중 '인천'에살거나 '부산'에 사는 직원을 출력하시오?(또는 : or / in)
...
select name as '이름', gender as '성별', age as '나이', address as '주소' from insarok where gender='여자' and age between 20 and 29;
...
select * from insarok where name like '_길_';
...
-- 영업과 사원을 출력?
select * from insarok where jikwi='사원' and buser='영업과';

-- '영업과', '사원'들에 대하여 직급을 모두 '대리'로 승진시키고, 기존급여에 50000추가하시오.
update insarok set jikwi='대리', netpay=netpay+50000 where  jikwi='사원' and buser='영업과';
